...
<template>
  <div class="m-auto mt-40 w-1/2">
    ...
    <select v-model="newExercise.type" class="m-2 block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
      ...
      <input type="date" v-model="newExercise.date" class="m-2 block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">

    <label v-if="newExercise.length === 'Time'" for="time">Time: (minutes) </label>
    ...
    <label v-if="newExercise.length === 'Sets'" for="sets">Sets:</label>
      ...
      <label v-if="newExercise.length === 'Sets'" for="weight">Weight: (lbs) </label>
    ...
    <!--submit button-->  <button class="shadow bg-purple-500 hover:bg-purple-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="submit">Add Exercise</button>
    </form>

    
  <!--editing section form-->
    <ul>
      <li v-for="(exercise, index) in exercises" :key="index">
       <template v-if="!exercise.isEditing">
        <span v-if="exercise.length === 'Time'">{{ exercise.date }} - {{ exercise.type }} - {{ exercise.time }} minutes</span>
<span v-else-if="exercise.length === 'Sets'">{{ exercise.date }} - {{ exercise.type }} - {{ exercise.sets }} sets - {{ exercise.reps }} reps - {{ exercise.weight }} lbs</span>
...
<template v-else>
  ...
  <select v-model="exercise.type" class="m-2 block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline" required>
             ...
             <input type="date" v-model="exercise.date" class="m-2 block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">

             <!--editing section labels-->
    <label v-if="exercise.length === 'Time'" for="time-edit">Time: (minutes)</label>
    ...
    <label v-if="exercise.length === 'Sets'" for="weight-edit">Weight: (lbs) </label>
